...
- Phase 4 : `server/discover`, métadonnées par requête, `tools/list`, `tools/call`, cache et accusé d'abonnement statique implémentés. Deux diagnostics en lecture seule. Tests XCTest et échange stdio passent ; MCP Inspector CLI 2.7.0 en mode moderne confirme discovery, liste et appel, désormais vérifiés en CI. Aucun Logic local.
