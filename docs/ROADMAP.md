...
- Phase 4 : `server/discover`, métadonnées par requête, `tools/list`, `tools/call`, cache et accusé d'abonnement statique implémentés. Compatibilité `initialize` 2025-11-25 ajoutée. Deux diagnostics en lecture seule, création et inspection d'un fichier MIDI format 1 sans Logic. Tests XCTest et échange stdio passent ; MCP Inspector CLI 2.7.0 confirme connexion, liste et appel dans les deux modes. Aucun Logic local.
- Phases 5 et 6 : procédure pilote écrite, avec essai d'import MIDI identique sur les deux Mac. Paquet préversion construit localement, non notarisé. Il faut encore les deux observations Logic, un assistant réellement utilisé par chaque pilote, des commandes directes qualifiées et une identité Developer ID pour une distribution sans friction Gatekeeper. MCP Inspector qualifie l'échange protocolaire, sans remplacer ces essais.
